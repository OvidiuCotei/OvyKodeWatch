...
class ProfileGenerator {
    ...
    public function createContentSection() {
        ...
        if(sizeof($videos) > 0)
        {
            ...
            $videGridHtml = $videoGrid->create($videos, null, false);
        }
        else
        {
            $videGridHtml = "<span>This user has no videos!</span>";
        }
    ...
    private function createHeaderButton() {
        if($this->userLoggedInObj->getUsername() == $this->profileData->getProfileUserFullName())
        {
            return "";
        }
        else
